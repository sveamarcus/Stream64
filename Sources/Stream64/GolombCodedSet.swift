...
public struct GolombCodedSetClient: Sendable {
    public init(
        decode: @escaping @Sendable ([UInt8], Int, Int) -> [UInt64]?,
...
enum Stream64GCS {
    ...
    static func decode(compressed: [UInt8], n: Int, p: Int) -> [UInt64]? {
        ...
        guard !compressed.isEmpty, n <= compressed.count * 8
            ...
            gcsLog.warning(
                "⚠️ GCS decode rejected — 🐘 implausible n=\(n) for \(compressed.count) B buffer (capping alloc, possible hostile filter) 🛡️"
            )
            return nil
        }
        ...
        if overflow != 0 {
            gcsLog.error(
                "💥 GCS decode aborted — ♾️ accumulator overflowed UInt64 after \(produced) value(s) (corrupt/hostile filter) 🧨"
            )
            return nil
        }
        guard produced == n
            ...
            gcsLog.trace(
                "⚠️ GCS decode truncated — 🪓 \(produced)/\(n) values (payload shorter than declared n) ✂️"
            )
            return nil
        }

        gcsLog.trace(
            "✅ GCS decode 📦 \(produced) values ⟵ \(compressed.count) B · ⏱️ \(Self.duration(elapsed)) · \(Self.rate(produced, elapsed)) 🎉"
        )
        return result
    }
    ...
    static func encode(sorted: [UInt64], p: Int) -> [UInt8]? {
        ...
        let (perElementBits, mulOverflow) = UInt64(sorted.count).multipliedReportingOverflow(
            by: UInt64(p + 1))
        ...
        guard !mulOverflow, !addOverflow1, !addOverflow2
            ...
            gcsLog.error(
                "❌ GCS encode rejected — 🌌 input too sparse to size a buffer for (count=\(sorted.count), p=\(p)) 🧱"
            )
            return nil
        }
        ...
        guard cRet == 0, !result.isEmpty
            ...
            gcsLog.error(
                "❌ GCS encode failed — 🧨 capacity exceeded (input not non-decreasing?) for \(sorted.count) values, p=\(p) 🚧"
            )
            return nil
        }

        gcsLog.debug(
            "✅ GCS encode 🗜️ \(sorted.count) values ⟶ \(result.count) B · \(Self.ratio(values: sorted.count, bytes: result.count)) · ⏱️ \(Self.duration(elapsed)) 🎁"
        )
        return result
    }
